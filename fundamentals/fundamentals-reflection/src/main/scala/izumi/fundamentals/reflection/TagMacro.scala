...
import izumi.fundamentals.reflection.macrortti.{LTag, LightTypeTagMacro0}
...
class TagMacro(val c: blackbox.Context) {
  ...
  def FIXMEgetLTagAlso[DIU <: Tags with Singleton, T](
                                                     // FIXME: TypeTag guard still required (can't call with DIU
                                                     //   FIXME(MOve out of CAKE)
                                                       t: c.Expr[DIU#ScalaReflectTypeTag[T]])(implicit w: c.WeakTypeTag[T]): c.Expr[DIU#Tag[T]] = {
    val tpe = w.tpe
    if (ReflectionUtil.allPartsStrong[c.universe.type](tpe.dealias)) {
      val ltagMacro = new LightTypeTagMacro0[c.type](c)
      val ltag = ltagMacro.makeParsedLightTypeTagImpl(tpe)
      c.Expr[DIU#Tag[T]] {
        q"${c.prefix.asInstanceOf[Expr[DIU#TagObject]]}.apply[$tpe](null, $ltag)"
      }
    } else {
