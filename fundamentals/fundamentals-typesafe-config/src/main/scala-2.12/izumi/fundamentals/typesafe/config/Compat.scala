...
private[config] object Compat {
  ...
  def objectMapReader(mirror: ru.Mirror, anyReader: SafeType0[ru.type] => ConfigReader[_])(mapType: ru.Type): ConfigReader[GenMap[String, _]] = {
      ...
      Try {
        ...
        mirror.reflectModule(mapType.dealias.companion.typeSymbol.asClass.module.asModule).instance match {
          case companionFactory: GenMapFactory[GenMap] @unchecked =>
            ...
            res
          case c =>
            throw new ConfigReadException(s"""When trying to read a Map type $mapType: can't instantiate class. Expected a companion object of type
    ...
    case cv =>
      Failure(new ConfigReadException(s"""Can't read config value as a map $mapType, config value is not an object.
                                         | ConfigValue was: $cv""".stripMargin))
  }

  def configListReader(mirror: ru.Mirror, anyReader: SafeType0[ru.type] => ConfigReader[_])(listType: ru.Type, cl: ConfigList): Try[GenTraversable[_]] = {
    Try {
      ...
      mirror.reflectModule(listType.dealias.companion.typeSymbol.asClass.module.asModule).instance match {
        ...
        case c =>
          throw new ConfigReadException(s"""When trying to read a collection type $listType: can't instantiate class. Expected a companion object of type
