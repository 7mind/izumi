...
class CliParserTest extends WordSpec {
  ...
  "CLI parser" should {
    "parse args" in {
      ...
      assert(new CLIParser().parse(Array("-x", "value")).exists(_.globalParameters.values.head == RawValue("x", "value")))
      assert(new CLIParser().parse(Array("--x", "value")).isLeft)
      assert(new CLIParser().parse(Array("--x=value")).exists(_.globalParameters.values.head == RawValue("x", "value")))
      assert(
        new CLIParser()
          .parse(Array(":init", "./tmp")) == Right(RawAppArgs(RawEntrypointParams.empty, Vector(RawRoleParams("init", RawEntrypointParams.empty, Vector("./tmp")))))
      )
      assert(
