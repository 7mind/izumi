...
import com.github.pshirshov.izumi.fundamentals.platform.time.IzTime.{ISO_DATE, ISO_DATE_TIME}
...
class IzTimeParsers(s: String) {
  ...
  def toTemporal: TemporalAccessor = ISO_DATE_TIME.parse(s)

  def toDate: TemporalAccessor = ISO_DATE.parse(s)

  def toTsZ: ZonedDateTime = ZonedDateTime.parse(s, ISO_DATE_TIME)
}
