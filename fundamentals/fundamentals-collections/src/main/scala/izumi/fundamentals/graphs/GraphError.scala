...
object ConflictResolutionError {
  ...
  case class AmbigiousActivationsSet[N](issues: Map[String, Set[AxisPoint]]) extends ConflictResolutionError[N]

  case class AmbigiousActivationDefs[N](node: Annotated[N], issues: Map[String, Set[AxisPoint]]) extends ConflictResolutionError[N]
