...
final class NonEmptyMap[K, +V] private (val toMap: Map[K, V]) extends AnyVal {
  ...
  /**
    * Returns a new <code>NonEmptyMap</code> containing the entries of this <code>NonEmptyMap</code> and the entries of the passed <code>Vector</code>.
    * The entry type of the resulting <code>NonEmptyMap</code> is the most specific superclass encompassing the entry types of this <code>NonEmptyMap</code> and the passed <code>Vector</code>.
    *
    * @tparam V1 the value type of the returned <code>NonEmptyMap</code>
    ...
    */
  def ++[V1 >: V](other: Vector[(K, V1)]): NonEmptyMap[K, V1] = new NonEmptyMap(toMap ++ other.toVector)

  // TODO: Have I added these extra ++, etc. methods to Vector that take a NonEmptyMap?

  /**
    * Returns a new <code>NonEmptyMap</code> containing the entries of this <code>NonEmptyMap</code> and the entries of the passed <code>TraversableOnce</code>.
  ...
  def ++[V1 >: V](other: IterableOnce[(K, V1)]): NonEmptyMap[K, V1] =
    if (other.iterator.isEmpty) this else new NonEmptyMap(toMap ++ other.iterator.toMap)
  ...
  /**
    * As with <code>++</code>, returns a new <code>NonEmptyMap</code> containing the entries of this <code>NonEmptyMap</code> and the entries of the passed <code>Vector</code>.
    * The entry type of the resulting <code>NonEmptyMap</code> is the most specific superclass encompassing the entry types of this <code>NonEmptyMap</code> and the passed <code>Vector</code>.
    *
    * It differs from <code>++</code> in that the right operand determines the type of the resulting collection rather than the left one. Mnemonic: the COLon is on the side of the new COLlection type.
    *
  ...
  /**
    ...
    */
  def sliding(size: Int): Iterator[NonEmptyMap[K, V]] = toMap.sliding(size).map(new NonEmptyMap(_))
  ...
  /**
    ...
    */
  def sliding(size: Int, step: Int): Iterator[NonEmptyMap[K, V]] = toMap.sliding(size, step).map(new NonEmptyMap(_))
  ...
  /**
    ...
    */
  def zipWithIndex[V1 >: V]: NonEmptyMap[(K, V1), Int] = new NonEmptyMap(toMap.zipWithIndex.toMap)
}
...
object NonEmptyMap {
  ...
  /**
    ...
    * @tparam K the type of the key contained in the new <code>NonEmptyMap</code>
    * @tparam V the type of the value contained in the new <code>NonEmptyMap</code>
    * @param seq the <code>Seq</code> with which to construct a <code>NonEmptyMap</code>
