...
import izumi.functional.bio.{Bifunctorized, SubmergedTypedError}
...
trait CatsTestEnv extends TestInstances with EqThrowable {
  ...
  // `TagK[cats.effect.IO]` for the `SubmergedTypedError[IO](...)` call sites below is
  // macro-derived at each call site (izumi-reflect supplies the macro). No explicit
  // `implicit val` is declared here because `TagK[X] = TagK[X]` resolves the summoner
  // method onto itself (forward reference / infinite loop on Scala 3).
  ...
  implicit def arbitraryIO2[A: Arbitrary: Cogen](implicit ticker: Ticker): Arbitrary[Bifunctorized[IO, Throwable, A]] = {
    Arbitrary(arbitraryIO[A].arbitrary.map { io =>
      Bifunctorized.assert[IO, Throwable, A](io.adaptError { case t => SubmergedTypedError[IO](t) })
    })
  }
