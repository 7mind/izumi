...
abstract class RetryPolicy[F[+_, +_]: Monad2, -A, +B](val action: RetryFunction[F, A, B]) {
  ...
  def >>>[B1](that: RetryPolicy[F, B, B1]): RetryPolicy[F, A, B1] = {
...
object RetryPolicy {
  ...
  def apply[F[+_, +_]: Monad2, A, B](action: RetryFunction[F, A, B]): RetryPolicy[F, A, B] = new RetryPolicy[F, A, B](action) {}

  def identity[F[+_, +_]: Monad2, A]: RetryPolicy[F, A, A] = {
    lazy val loop: RetryFunction[F, A, A] = (now: ZonedDateTime, in: A) => F.pure(ControllerDecision.Repeat(in, now, loop))
    RetryPolicy(loop)
  }

  def giveUp[F[+_, +_]: Monad2]: RetryPolicy[F, Any, Unit] = {
    RetryPolicy(RetryFunction.done(()))
  }

  def elapsed[F[+_, +_]: Monad2]: RetryPolicy[F, Any, FiniteDuration] = {
  ...
  def forever[F[+_, +_]: Monad2]: RetryPolicy[F, Any, Long] = {
    def loop(n: Long): RetryFunction[F, Any, Long] = (now, _) => F.pure(ControllerDecision.Repeat(n, now, loop(n + 1L)))
    RetryPolicy(loop(0))
  }

  def recurs[F[+_, +_]: Monad2](n: Int): RetryPolicy[F, Any, Long] = {
    forever[F].whileOutput(_ < n)
  }

  def spaced[F[+_, +_]: Monad2](timeInterval: FiniteDuration): RetryPolicy[F, Any, Long] = {
    forever[F].modifyDelay(_ => timeInterval)
  }

  def recursWhile[F[+_, +_]: Monad2, A](f: A => Boolean): RetryPolicy[F, A, A] = {
    identity[F, A].whileInput(f)
  }

  def fixed[F[+_, +_]: Monad2](period: FiniteDuration): RetryPolicy[F, Any, Long] = {
  ...
  def exponential[F[+_, +_]: Monad2](baseDelay: FiniteDuration): RetryPolicy[F, Any, FiniteDuration] = {
    forever.map(retriesSoFar => multiply(baseDelay, Math.pow(2.0, retriesSoFar.toDouble).toLong)).modifyDelay(d => d)
  }

  def exponentialWithBound[F[+_, +_]: Monad2](baseDelay: FiniteDuration, maxRetries: Int): RetryPolicy[F, Any, (FiniteDuration, Long)] = {
  ...
  object RetryFunction {
    def done[F[+_, +_]: Monad2, A](value: A): RetryFunction[F, Any, A] = (_, _) => F.pure(ControllerDecision.Stop(value))
  }
