...
class PrimitivesFromBIOAndCats[F[+_, +_]: Async2: Fork2] extends Primitives2[F] {
  private val Concurrent: Concurrent[F[Throwable, _]] = catz.BIOAsyncForkToConcurrent
  ...
  override def mkPromise[E, A]: F[Nothing, Promise2[F, E, A]] = {
    Deferred.tryable[F[Throwable, _], F[E, A]](Concurrent).map(Promise2.fromCats[F, E, A]).orTerminate
  }
