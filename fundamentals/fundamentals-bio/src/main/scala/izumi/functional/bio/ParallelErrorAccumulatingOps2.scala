...
trait ParallelErrorAccumulatingOps2[F[+_, +_]] { P: Parallel2[F] =>
  def InnerF: Error2[F]
  ...
  def parTraverseAccumErrors[ColL[_], E, A, B](
    ...
    implicit val F: Error2[F] = (this: ParallelErrorAccumulatingOps2[F]).InnerF

    P.parTraverse(col)(F `attempt` f(_)).flatMap {
