...
lazy val `fundamentals-collections` = project
  .in(file("fundamentals/fundamentals-collections"))
  .dependsOn(
...
lazy val `fundamentals-platform` = project
  .in(file("fundamentals/fundamentals-platform"))
  .dependsOn(
...
lazy val `fundamentals-language` = project
  .in(file("fundamentals/fundamentals-language"))
  .settings(
    libraryDependencies ++= Seq(
        compilerPlugin("org.typelevel" % "kind-projector" % V.kind_projector cross CrossVersion.full),
        compilerPlugin("com.softwaremill.neme" %% "neme-plugin" % V.neme_plugin),
...
lazy val `fundamentals-reflection` = project
  .in(file("fundamentals/fundamentals-reflection"))
  .dependsOn(
...
lazy val `fundamentals-functional` = project
  .in(file("fundamentals/fundamentals-functional"))
  .settings(
    libraryDependencies ++= Seq(
        compilerPlugin("org.typelevel" % "kind-projector" % V.kind_projector cross CrossVersion.full),
        compilerPlugin("com.softwaremill.neme" %% "neme-plugin" % V.neme_plugin),
...
lazy val `fundamentals-bio` = project
  .in(file("fundamentals/fundamentals-bio"))
  .dependsOn(
    `fundamentals-language` % "test->compile;compile->compile"
  )
  .settings(
...
lazy val `fundamentals-json-circe` = project
  .in(file("fundamentals/fundamentals-json-circe"))
  .dependsOn(
    `fundamentals-platform` % "test->compile;compile->compile"
  )
  .settings(
...
lazy val `distage-core-api` = project
  .in(file("distage/distage-core-api"))
  .dependsOn(
...
lazy val `distage-core-proxy-cglib` = project
  .in(file("distage/distage-core-proxy-cglib"))
  .dependsOn(
...
lazy val `distage-core` = project
  .in(file("distage/distage-core"))
  .dependsOn(
    `distage-core-api` % "test->compile;compile->compile",
...
lazy val `distage-extension-config` = project
  .in(file("distage/distage-extension-config"))
  .dependsOn(
    `distage-core-api` % "test->compile;compile->compile",
...
lazy val `distage-extension-plugins` = project
  .in(file("distage/distage-extension-plugins"))
  .dependsOn(
    `distage-core-api` % "test->compile;compile->compile",
    `distage-core` % "test->compile",
...
lazy val `distage-extension-logstage` = project
  .in(file("distage/distage-extension-logstage"))
  .dependsOn(
...
lazy val `distage-framework-api` = project
  .in(file("distage/distage-framework-api"))
  .dependsOn(
    `distage-core-api` % "test->compile;compile->compile"
  )
  .settings(
...
lazy val `distage-framework` = project
  .in(file("distage/distage-framework"))
  .dependsOn(
...
lazy val `distage-framework-docker` = project
  .in(file("distage/distage-framework-docker"))
  .dependsOn(
    `distage-core` % "test->compile;compile->compile",
    `distage-extension-config` % "test->compile;compile->compile",
    `distage-framework-api` % "test->compile;compile->compile",
...
lazy val `distage-testkit-core` = project
  .in(file("distage/distage-testkit-core"))
  .dependsOn(
    `distage-extension-config` % "test->compile;compile->compile",
...
lazy val `distage-testkit-scalatest` = project
  .in(file("distage/distage-testkit-scalatest"))
  .dependsOn(
...
lazy val `testapp-launcher` = project
  .in(file("distage/testapp-launcher"))
  .dependsOn(
    `testapp` % "test->compile;compile->compile"
  )
  .settings(
    libraryDependencies ++= Seq(
        compilerPlugin("org.typelevel" % "kind-projector" % V.kind_projector cross CrossVersion.full),
...
lazy val `logstage-core` = project
  .in(file("logstage/logstage-core"))
  .dependsOn(
    `fundamentals-bio` % "test->compile;compile->compile",
    `fundamentals-platform` % "test->compile;compile->compile",
  )
...
lazy val `logstage-rendering-circe` = project
  .in(file("logstage/logstage-rendering-circe"))
  .dependsOn(
...
lazy val `logstage-adapter-slf4j` = project
  .in(file("logstage/logstage-adapter-slf4j"))
  .dependsOn(
    `logstage-core` % "test->test;compile->compile"
  )
  .settings(
...
lazy val `logstage-sink-slf4j` = project
  .in(file("logstage/logstage-sink-slf4j"))
  .dependsOn(
    `logstage-core` % "test->test;compile->compile"
  )
  .settings(
...
lazy val `microsite` = project
  ...
  .dependsOn(
    ...
    `testapp` % "test->compile;compile->compile",
    `testapp-launcher` % "test->compile;compile->compile",
  ...
  .settings(
    ...
    unidocProjectFilter in (ScalaUnidoc, unidoc) := inAggregates(`fundamentals-jvm`, transitive = true) || inAggregates(`distage-jvm`, transitive = true) || inAggregates(
        `logstage-jvm`,
        transitive = true,
      ),
...
lazy val `sbt-izumi-deps` = project
  .in(file("sbt-plugins/sbt-izumi-deps"))
  .settings(
    libraryDependencies ++= Seq(
        compilerPlugin("org.typelevel" % "kind-projector" % V.kind_projector cross CrossVersion.full),
        compilerPlugin("com.softwaremill.neme" %% "neme-plugin" % V.neme_plugin),
