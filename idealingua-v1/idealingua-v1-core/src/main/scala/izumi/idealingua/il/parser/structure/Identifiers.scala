...
trait Identifiers extends Separators {
  ...
  def idGeneric[_: P]: P[AbstractIndefiniteId] =
    P(inline ~ identifier ~ inline ~ generic.rep(min = 0, max = 1) ~ inline)
  ...
  def justString[_: P]: P[InterpContext] =
    P("t\"" ~ staticPart ~ "\"")
