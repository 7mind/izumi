...
class DefStructure(context: IDLParserContext) extends Separators {
  ...
  def field[_: P]: P[RawField] = P(metaAgg.withMeta((ids.symbol | P("_" | "").map(_ => "")) ~ inline ~ ":" ~/ inline ~ ids.idGeneric)).map {
  ...
  def aggregate[_: P]: P[Seq[RawField]] =
    P(
      (inline ~ field ~ inline)
        .rep(sep = sepStruct)
    )

  def nestedAdtMember[_: P]: P[Member.NestedDefn] = P(defMember.baseTypeMember).map {
    m =>
      Member.NestedDefn(m.v)
  }

  def adtMember[_: P]: P[Member.TypeRef] = P(metaAgg.withMeta(ids.identifier ~ (inline ~ "as" ~/ (inline ~ ids.symbol)).?)).map {
    case (meta, (tpe, alias)) =>
  ...
  def adt[_: P](sep: => P[Unit]): P[RawAdt] =
    P((nestedAdtMember | adtMember).rep(min = 1, sep = sep))
  ...
  def dtoBlock[_: P]: P[DTO] = P(metaAgg.cblock(kw.data, Struct.struct)).map {
  ...
  def foreignBlock[_: P]: P[RawTopLevelDefn.TLDForeignType] =
    P(metaAgg.withMeta(kw(kw.foreign, ids.idGeneric ~ inline ~ foreignLinks))).map {
      case (meta, (i, v)) =>
        ForeignType(i, v, meta)
    }.map(RawTopLevelDefn.TLDForeignType)

  def idBlock[_: P]: P[Identifier] = P(metaAgg.cblock(kw.id, aggregate)).map {
    case (c, i, v) => Identifier(i.toIdId, v.toList, c)
  }

  def aliasBlock[_: P]: P[Alias] = P(metaAgg.cstarting(kw.alias, "=" ~/ (inline ~ ids.identifier))).map {
    case (c, i, v) => Alias(i.toAliasId, v.toIndefinite, c)
  }

  def declaredBlock[_: P]: P[TLDDeclared] = P(metaAgg.cstarting(kw.declared, inline)).map {
    case (meta, id, _) =>
      TLDDeclared(DeclaredType(id.toIndefinite, meta))
  }

  def cloneBlock[_: P]: P[TLDNewtype] =
    P(metaAgg.cstarting(kw.newtype, "into" ~/ (inline ~ ids.idShort ~ inline ~ aggregates.enclosed(Struct.struct).?))).map {
  ...
  def adtBlock[_: P]: P[Adt] = P(metaAgg.cstarting(kw.adt, adtEnclosed | adtFreeForm)).map {
    case (c, i, v) =>
  ...
  def enumBlock[_: P]: P[Enumeration] = P(metaAgg.cstarting(kw.enum, enumEnclosed | enumFreeForm)).map {
    case (c, i, v) =>
