...
object JsonNetExtension extends CSharpTranslatorExtension {
  ...
  private def prepareReadPropertyValue(src: String, dst: String, i: CSharpType, createDst: Boolean, currentDomain: String)(
      ...
      i.id match {
        ...
        case o: Generic.TOption =>
          ...
          Some(s"""${i.renderType(true)} $dst = null;
                  ...
                  |${(if (propertyNeedsPrepare(o.valueType)) prepareReadPropertyValue(src, dst, ot, createDst = false, currentDomain).get
                   else s"$dst = ${readPropertyValue(src, ot, currentDomain)};").shift(4)}
                  |}
             """.stripMargin)
...
//          )
  ...
  override def postModelEmit(ctx: CSTContext, i: Adt)(implicit im: CSharpImports, ts: Typespace): String = {
    ...
    s"""public class ${i.id.name}_JsonNetConverter: JsonNetConverter<${i.id.name}> {
       ...
       |        writer.WriteStartObject();
       |${i.alternatives.map(m => s"""if (al is ${i.id.name}.${m.wireId}) {
       ...
       |        var kv = json.Properties().First();
       |        switch (kv.Name) {
       |${i.alternatives.map(m => s"""case "${m.wireId}": {
  ...
  override def postModelEmit(ctx: CSTContext, name: String, alternative: Alternative, leftType: TypeId, rightType: TypeId)(
    implicit im: CSharpImports,
    ts: Typespace
  ): String = {
    discard(ctx)
