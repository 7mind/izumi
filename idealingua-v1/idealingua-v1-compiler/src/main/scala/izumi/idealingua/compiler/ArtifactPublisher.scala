...
class ArtifactPublisher(targetDir: Path, lang: IDLLanguage, creds: Credentials, manifest: BuildManifest) {
  ...
  private def publishScala(targetDir: Path, creds: ScalaCredentials): Either[Throwable, Unit] =
    Try {
  ...
  private def publishTypescript(targetDir: Path, creds: TypescriptCredentials): Either[Throwable, Unit] =
    Try {
      ...
      Files.list(targetDir.resolve("dist")).filter(_.toFile.isDirectory).iterator().asScala.foreach {
        module =>
  ...
  private def publishCsharp(targetDir: Path, creds: CsharpCredentials): Either[Throwable, Unit] =
    Try {
  ...
  private def publishGo(targetDir: Path, creds: GoCredentials, manifest: GoLangBuildManifest): Either[Throwable, Unit] =
    Try {
