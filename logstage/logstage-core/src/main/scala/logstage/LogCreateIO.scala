...
trait LogCreateIO[F[_]] {
  ...
  def widen[G[_]](implicit @unused ev: F[Unit] <:< G[Unit]): LogCreateIO[G] = this.asInstanceOf[LogCreateIO[G]]
}
...
object LogCreateIO extends LowPriorityLogCreateIOInstances {
  ...
  implicit def covarianceConversion[G[_], F[_]](log: LogCreateIO[F])(implicit ev: F[Unit] <:< G[Unit]): LogCreateIO[G] = log.widen
}
