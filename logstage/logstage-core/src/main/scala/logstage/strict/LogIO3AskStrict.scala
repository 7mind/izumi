...
//  @inline def make[F[-_, +_, +_]: MonadAsk3](implicit t: Tag[LogIO3Strict[F]]): LogIO3AskStrict[F] = new LogIO3AskStrictImpl[F](_.get[LogIO3Strict[F]])
  @inline def make[F[-_, +_, +_]: MonadAsk3]: LogIO3AskStrict[F] = new LogIO3AskStrictImpl[F](identity)
  ...
  class LogIO3AskStrictImpl[F[-_, +_, +_]](
    get: LogIO3Strict[F] => LogIO3Strict[F]
  )(implicit
    F: MonadAsk3[F]
  ) extends LogIO3AskStrict[F] {
    override final def log(entry: Log.Entry): F[LogIO3Strict[F], Nothing, Unit] =
      F.access(get(_).log(entry))
    override final def log(logLevel: Level)(messageThunk: => Log.Message)(implicit pos: CodePositionMaterializer): F[LogIO3Strict[F], Nothing, Unit] =
      F.access(get(_).log(logLevel)(messageThunk))
    override final def unsafeLog(entry: Log.Entry): F[LogIO3Strict[F], Nothing, Unit] =
      F.access(get(_).log(entry))
    override final def acceptable(loggerId: Log.LoggerId, logLevel: Level): F[LogIO3Strict[F], Nothing, Boolean] =
      F.access(get(_).acceptable(loggerId, logLevel))
    override final def acceptable(logLevel: Level)(implicit pos: CodePositionMaterializer): F[LogIO3Strict[F], Nothing, Boolean] =
      F.access(get(_).acceptable(logLevel))
    override final def createEntry(logLevel: Level, message: Log.Message)(implicit pos: CodePositionMaterializer): F[LogIO3Strict[F], Nothing, Log.Entry] =
      F.access(get(_).createEntry(logLevel, message))
    override final def createContext(
      logLevel: Level,
      customContext: CustomContext,
    )(implicit pos: CodePositionMaterializer
    ): F[LogIO3Strict[F], Nothing, Log.Context] =
      F.access(get(_).createContext(logLevel, customContext))
    override final def withCustomContext(context: CustomContext): LogIO2Strict[F[LogIO3Strict[F], _, _]] = {
