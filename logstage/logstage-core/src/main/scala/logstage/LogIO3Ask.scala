...
object LogIO3Ask {
  ...
  class LogIO3AskImpl[F[-_, +_, +_]](
    ...
    override final def log(entry: Log.Entry): F[Has[LogIO3[F]], Nothing, Unit] =
      F.access(get(_).log(entry))
    override final def log(logLevel: Level)(messageThunk: => Log.Message)(implicit pos: CodePositionMaterializer): F[Has[LogIO3[F]], Nothing, Unit] =
      F.access(get(_).log(logLevel)(messageThunk))
    override final def unsafeLog(entry: Log.Entry): F[Has[LogIO3[F]], Nothing, Unit] =
      F.access(get(_).log(entry))
    override final def acceptable(loggerId: Log.LoggerId, logLevel: Level): F[Has[LogIO3[F]], Nothing, Boolean] =
      F.access(get(_).acceptable(loggerId, logLevel))
    override final def acceptable(logLevel: Level)(implicit pos: CodePositionMaterializer): F[Has[LogIO3[F]], Nothing, Boolean] =
      F.access(get(_).acceptable(logLevel))
    override final def createEntry(logLevel: Level, message: Log.Message)(implicit pos: CodePositionMaterializer): F[Has[LogIO3[F]], Nothing, Log.Entry] =
    ...
    override final def withCustomContext(context: CustomContext): LogIO2[F[Has[LogIO3[F]], _, _]] = {
