...
open class PluginLoaderClassgraphImpl extends PluginLoader {
  ...
  protected def scanClasspath(config: PluginConfig): Seq[PluginBase] = {
    ...
    def loadPkgs(pkgs: Seq[String]): Seq[PluginBase] = {
      PluginLoaderClassgraphImpl.doLoad[PluginBase](pluginBase.getName, whitelistedClasses, pkgs, disabledPackages, config.debug)
    }
...
object PluginLoaderClassgraphImpl {
  ...
  def doLoad[T](base: String, whitelistClasses: Seq[String], enabledPackages: Seq[String], disabledPackages: Seq[String], debug: Boolean): Seq[T] = {
    val scanResult = new ClassGraph()
      .acceptPackages(enabledPackages*)
      .acceptClasses((whitelistClasses :+ base)*)
      .rejectPackages(disabledPackages*)
