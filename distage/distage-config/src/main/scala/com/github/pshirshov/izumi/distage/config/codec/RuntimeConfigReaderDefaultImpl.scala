...
(
  ...
  , configReaders: Set[RuntimeConfigReaderCodecs]
) extends RuntimeConfigReader {

  private val codecs = configReaders.flatMap(_.readerCodecs).toMap
...
trait RuntimeConfigReaderCodecs {
  def readerCodecs: Map[SafeType, ConfigReader[_]]
}
