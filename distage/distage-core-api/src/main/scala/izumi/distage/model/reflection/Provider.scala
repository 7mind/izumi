...
object Provider {
  ...
  final case class ProviderImpl[+A](
    ...
    def this(parameters: Seq[Association.Parameter], ret: SafeType, fun: Seq[Any] => Any, providerType: ProviderType) =
    ...
    override def addUnused(keys: Seq[DIKey]): Provider =
      copy(
        parameters = parameters ++ keys.map(key => Association.Parameter(SymbolInfo("<unused>", key.tpe, isByName = false, wasGeneric = false), key)),
  ...
  object ProviderImpl {
    @inline def apply[A](parameters: Seq[Association.Parameter], ret: SafeType, fun: Seq[Any] => Any, providerType: ProviderType): ProviderImpl[A] =
