  ...
  object MakeDSLBase extends AnyKindShim {
    implicit final class MakeFromZIOHas[T, AfterBind](protected val dsl: MakeDSLBase[T, AfterBind]) extends AnyVal with MakeFromHasLowPriorityOverloads[T, AfterBind] {
      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](effect: ZIO[R, E, I]): AfterBind = {
        dsl.fromEffect[IO[E, _], I](HasConstructor[R].map(effect.provideEnvironment(_)))
      }
      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](function: Functoid[ZIO[R, E, I]]): AfterBind = {
        dsl.fromEffect[IO[E, _], I](function.map2(HasConstructor[R])(_.provideEnvironment(_)))
      }

      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](resource: ZManaged[R, E, I]): AfterBind = {
        dsl.fromResource(HasConstructor[R].map(resource.provideEnvironment(_)))
      }
      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](function: Functoid[ZManaged[R, E, I]])(implicit d1: DummyImplicit): AfterBind = {
        dsl.fromResource(function.map2(HasConstructor[R])(_.provideEnvironment(_)))
      }

      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](layer: ZLayer[R, E, I]): AfterBind = {
        dsl.fromResource(HasConstructor[R].map(ZLayer.succeedEnvironment(_) >>> layer))
      }
      def fromHas[R: HasConstructor, E: Tag, I <: T: Tag](function: Functoid[ZLayer[R, E, I]])(implicit d1: DummyImplicit, d2: DummyImplicit): AfterBind = {
        dsl.fromResource(function.map2(HasConstructor[R])((layer, e) => ZLayer.succeedEnvironment(e) >>> layer))
      }
  ...
  object SetDSLBase extends AnyKindShim {
    implicit final class AddFromZIOHas[T, AfterAdd](protected val dsl: SetDSLBase[T, AfterAdd, ?]) extends AnyVal with AddFromHasLowPriorityOverloads[T, AfterAdd] {
      ...
      def addHas[R: HasConstructor, E: Tag, I <: T: Tag](
        ...
        dsl.addResource(function.map2(HasConstructor[R])(_.provideEnvironment(_)))
      }

      def addHas[R: HasConstructor, E: Tag, I <: T: Tag](layer: ZLayer[R, E, I])(implicit pos: CodePositionMaterializer): AfterAdd = {
        dsl.addResource(HasConstructor[R].map(ZLayer.succeedEnvironment(_) >>> layer))
      }
      def addHas[R: HasConstructor, E: Tag, I <: T: Tag](
        ...
        dsl.addResource(function.map2(HasConstructor[R])((r, e) => ZLayer.succeedEnvironment(e) >>> r))
      }
