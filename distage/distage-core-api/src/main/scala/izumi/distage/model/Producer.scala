...
/** Executes instructions in [[izumi.distage.model.plan.Plan]] to produce a [[izumi.distage.model.Locator]]
  ...
  */
trait Producer {
  private[distage] def produceDetailedFX[F[+_, +_]: TagKK: IO2](
    plan: Plan,
    filter: FinalizerFilter[F],
  )(implicit tkFThrowable: TagK[F[Throwable, _]]
  ): Lifecycle[F, Throwable, Either[FailedProvision, Locator]]
  private[distage] final def produceFX[F[+_, +_]: TagKK: IO2: Primitives2](
    plan: Plan,
    filter: FinalizerFilter[F],
  )(implicit tkFThrowable: TagK[F[Throwable, _]]
  ...
  final def produceCustomF[F[+_, +_]: TagKK: IO2: Primitives2](
    plan: Plan
  )(implicit tkFThrowable: TagK[F[Throwable, _]]
  ): Lifecycle[F, Throwable, Locator] = {
    produceFX[F](plan, FinalizerFilter.all[F])
  }
  final def produceDetailedCustomF[F[+_, +_]: TagKK: IO2](
    plan: Plan
  )(implicit tkFThrowable: TagK[F[Throwable, _]]
  ): Lifecycle[F, Throwable, Either[FailedProvision, Locator]] = {
