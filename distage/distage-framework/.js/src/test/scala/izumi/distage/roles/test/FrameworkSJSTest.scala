...
class FrameworkSJSTest extends AsyncWordSpec { self =>
        ...
        override protected def pluginConfig: PluginConfig = PluginConfig.const(
          Seq(
            new PluginDef with RoleModuleDef {
              ...
              make[ExecutionContext].named("cpu").from(self.executionContext)
              make[Executor].named("cpu").from(Executor.fromExecutionContext(self.executionContext))
            }
          )
        )
