...
object PlanCheckMaterializer {
  ...
  object PlanCheckMaterializerMacro extends PlanCheckMaterializerCommon {
    def impl[
      ...
      val warnOnly = onlyWarn.getOrElse(PlanCheck.defaultOnlyWarn)
      ...
      val (maybeMessage, checkedLoadedPlugins) = {
        doCheckApp(roles, activations, config, checkConfig, printBindings, warnOnly, maybeMain, logger)
      }
      val checkPassed = maybeMessage match {
        case None => true
        case Some(message) =>
          if (warnOnly) {
            val pkg = SourcePackageMaterializerMacro.getSourcePackageString(c)
            val fatalWarnings = c.compilerSettings.exists(s => s == "-Wconf:any:error" || s == "-Xfatal-warnings")
            if (pkg == "izumi.distage.testkit.distagesuite.compiletime") {
              // silence test output
              ()
