  ...
  object runtime {
    ...
    def checkAnyApp[F[_]](
      ...
      val configIssues = if (checkConfig) {
        ...
        reportEffectiveConfig(DistageConfigImpl.maybeOriginDescription(realAppConfig.config).getOrElse {
          configLoader match {
            case sc: RoleCheckableApp[_]#SpecificResourceConfigLoader =>
              s"resource:${sc.resourceName}"
            case _ =>
              "<origin unavailable>"
          }
        })
