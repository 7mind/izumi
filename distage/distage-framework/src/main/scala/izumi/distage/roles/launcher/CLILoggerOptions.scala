...
object CLILoggerOptionsReader {
  class CLILoggerOptionsReaderImpl(
    ...
    private def getRootLogLevel(parameters: EntrypointArgs, defaultLogLevel: Log.Level): Level = {
      parameters
    ...
    private def getLogFormatJson(parameters: EntrypointArgs, defaultLogFormatJson: Boolean): Boolean = {
      parameters
