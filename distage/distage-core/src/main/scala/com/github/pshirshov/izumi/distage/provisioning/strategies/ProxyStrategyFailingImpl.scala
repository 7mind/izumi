...
class ProxyStrategyFailingImpl extends ProxyStrategy {
  override def initProxy(context: ProvisioningKeyProvider, executor: OperationExecutor, initProxy: ProxyOp.InitProxy): Seq[ContextAssignment] = {
  ...
  override def makeProxy(context: ProvisioningKeyProvider, makeProxy: ProxyOp.MakeProxy): Seq[ContextAssignment] = {
