...
class ClassStrategyDefaultImpl(
      ...
      tpe =>
        ...
        if (symbol.isModule) { // don't re-instantiate scala objects
          ...
          if (hasByName) {
            ...
            val refCtor = refClass.reflectConstructor(
              ctorSymbol.getOrElse(
  ...
  protected def reflectClass(context: ProvisioningKeyProvider, op: WiringOp.InstantiateClass, symbol: Symbol): (ClassMirror, Option[Any]) = {
    ...
    val targetType = wiring.instanceType
    if (!symbol.isStatic) {
      val typeRef = targetType.use(
        tpe =>
          ReflectionUtil
            .toTypeRef[u.type](tpe)
  ...
  protected def mkJava(targetType: SafeType, prefix: Option[Any], args: Seq[Dep]): Any = {
    ...
    val argValues = prefix.map(_.asInstanceOf[AnyRef]).toSeq ++ args.map {
        case Dep(key, value) =>
          unboxingTool.unbox(key, value)
      }

    val allConstructors = clazz.map(_.getDeclaredConstructors).toList.flatten

    val sameArityConstructors = allConstructors
      .filter(_.getParameterCount == argValues.size)

    val matchingConstructors = sameArityConstructors.find {
      c =>
    ...
    matchingConstructors match {
      ...
      case None =>
        ...
        val discrepancies = sameArityConstructors.map {
          c =>
            val repr = c.getParameterTypes.toList
              .zip(argValues).map({ case (exp, impl) => s"expected: $exp, have: ${impl.getClass}, matches: ${TypeUtil.isAssignableFrom(exp, impl)}" }).niceList().shift(2)

            s"$c: $repr"
        }

        throw new ProvisioningException(s"""Can't find constructor for $targetType in class $clazz, signature: ${argValues.map(_.getClass)}
