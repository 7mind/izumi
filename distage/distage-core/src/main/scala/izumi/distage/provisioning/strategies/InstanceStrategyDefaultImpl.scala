...
class InstanceStrategyDefaultImpl extends InstanceStrategy {
  def getInstance[F[_]: TagK](context: ProvisioningKeyProvider, op: WiringOp.UseInstance)(implicit F: IO1[F]): F[Either[ProvisionerIssue, Seq[NewObjectOp]]] = {
    F.pure(Right(Seq(NewObjectOp.NewInstance(op.target, op.instanceType, op.wiring.instance))))
  }
  def getInstance[F[_]: TagK](context: ProvisioningKeyProvider, op: WiringOp.ReferenceKey)(implicit F: IO1[F]): F[Either[ProvisionerIssue, Seq[NewObjectOp]]] = {
