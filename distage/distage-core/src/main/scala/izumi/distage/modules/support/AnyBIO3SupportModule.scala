...
/** Any `BIO` effect type support for `distage` resources, effects, roles & tests.
  ...
  *
  * - Adds [[izumi.distage.model.effect.QuasiEffect]] instances to support using `F[-_, +_, +_]` in `Injector`, `distage-framework` & `distage-testkit-scalatest`
...
class AnyBIO3SupportModule[F[-_, +_, +_]: TagK3](implicit tagBIO: TagKK[F[Any, +?, +?]]) extends ModuleDef {
  // QuasiEffect & bifunctor bio instances
  ...
  def withImplicits[F[-_, +_, +_]: TagK3: BIOAsync3: BIOTemporal3: BIOLocal: BIORunner3: BIOFork3: BIOPrimitives3](implicit tagBIO: TagKK[F[Any, +?, +?]]): ModuleDef =
    new ModuleDef {
      ...
      private[this] def aliasingCheck(): Unit = {
        ...
        implicitly[QuasiEffectRunner3[F] =:= QuasiEffectRunner2[F[Any, +?, +?]]]
        implicitly[QuasiEffect3[F] =:= QuasiEffect2[F[Any, +?, +?]]]
        implicitly[QuasiApplicative3[F] =:= QuasiApplicative2[F[Any, +?, +?]]]
        implicitly[QuasiEffectAsync3[F] =:= QuasiEffectAsync2[F[Any, +?, +?]]]
        ()
      }
    }
}
