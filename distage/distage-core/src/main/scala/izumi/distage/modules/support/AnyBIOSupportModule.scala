...
object AnyBIOSupportModule {
  ...
  /**
    ...
    */
  def usingImplicits[F[+_, +_]: TagKK: Async2: Temporal2: UnsafeRun2: BlockingIO2: Fork2: Primitives2: PrimitivesM2: PrimitivesLocal2: Scheduler2](
    implicit t: TagK[F[Throwable, _]],
    tn: TagK[F[Nothing, _]],
  ): ModuleDef = new ModuleDef {
    include(AnyBIOSupportModule.usingDependencies[F])
    include(BIOInstancesModule.usingAsyncTemporalAuxAlgebrasImplicits[F])
  }

  @deprecated("renamed to usingDependencies", "1.3")
  @inline def apply[F[+_, +_]: TagKK](implicit t: TagK[F[Throwable, _]], tn: TagK[F[Nothing, _]]): ModuleDef = usingDependencies[F]
}
