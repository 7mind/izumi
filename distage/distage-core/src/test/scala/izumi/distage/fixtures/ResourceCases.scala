...
object ResourceCases {
  ...
  object CircularResourceCase {
    ...
    def s3ComponentResource[F[_]: IO1](ref: Ref[F, Queue[Ops]], s3Client: S3Client): Lifecycle[F, S3Component] =
      Lifecycle.make(
        acquire = ref.update(_ :+ ComponentStart).map(_ => new S3Component(s3Client))
      )(release = _ => ref.update_(_ :+ ComponentStop))

    def s3clientResource[F[_]: IO1](ref: Ref[F, Queue[Ops]], s3Component: S3Component): Lifecycle[F, S3ClientImpl] =
      Lifecycle.make(
  ...
  class Ref[F[_], A](r: AtomicReference[A])(implicit F: IO1[F]) {
  ...
  object Ref {
    ...
    final class Apply[F[_]](private val dummy: Boolean = false) extends AnyVal {
      def apply[A](a: A)(implicit F: IO1[F]): F[Ref[F, A]] = {
        IO1[F].maybeSuspend(new Ref[F, A](new AtomicReference(a)))
      }
    }
  }
  ...
  object Suspend2 {
    ...
    implicit def IO1Suspend2[E <: Throwable]: IO1[Suspend2[E, _]] = new IO1[Suspend2[E, _]] {
