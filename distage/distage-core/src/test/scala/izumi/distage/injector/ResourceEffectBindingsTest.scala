...
class ResourceEffectBindingsTest extends WordSpec with MkInjector {
    ...
    "deallocate correctly on error in nested .flatMap" in {
      ...
      Try {
        DIResource
          ...
          .flatMap {
            _ =>
              DIResource.makeSimple(ops1 += YStart)(_ => ops1 += YStop).flatMap {
                _ =>
      ...
      Try {
        DIResource
          ...
          .flatMap {
            _ =>
              DIResource.make(Suspend2(ops2 += YStart))(_ => Suspend2(ops2 += YStop).void).flatMap {
                _ =>
  ...
  "Resource bindings" should {
    ...
    "deallocate correctly in case of exceptions" in {
      ...
      val resource = injector.produceDetailedF[Suspend2[Throwable, ?]](plan).evalMap {
        case Left(failure) =>
          Suspend2 {
