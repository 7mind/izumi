...
class AxisTest extends AnyWordSpec with MkInjector {
  ...
  "choose according to specificity rules" in {
    ...
    def DefaultsModule = new ModuleDef {
      make[Color].fromImplicit(Green)
      make[Color].tagged(Style.AllCaps).fromImplicit(RED)
    }
    ...
    def SpecificityModule = new ModuleDef {
      make[Color].tagged(Mode.Test).fromImplicit(Blue)
      make[Color].tagged(Mode.Prod).fromImplicit(Green)
      make[Color].tagged(Mode.Prod, Style.AllCaps).fromImplicit(RED)
    }
