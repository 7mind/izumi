...
class HigherKindsTest extends AnyWordSpec with MkInjector {
  ...
  "support tagless final style module definitions" in {
    ...
    case class Definition[F[_]: TagK: Pointed](getResult: Int) extends ModuleDef {
      ...
      make[Either[F[String], F[F[F[F[String]]]]]].fromValue(Right(Pointed[F].point(Pointed[F].point(Pointed[F].point(Pointed[F].point("aaa"))))))

      make[F[Nothing]].fromValue(null.asInstanceOf[F[Nothing]])
    ...
    assert(eitherContext.get[TestServiceTrait[Either[String, _]]].get == Right(10))
  ...
  "TagKK works" in {
    ...
    class Definition[F[+_, +_]: TagKK: TestCovariantTC, G[_]: TagK, A: Tag](v: F[String, Int]) extends ModuleDef {
      ...
      make[F[String, Int]].fromValue(v)
    }

    val value: Either[String, Int] = Right(5)
    val definition = new Definition[Either, Option, Int](value)

    val context = Injector.Standard().produce(PlannerInput.everything(definition)).unsafeGet()
    assert(context != null)
