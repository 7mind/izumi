...
object Association {
  ...
  case class Parameter(context: DependencyContext.ParameterContext, symbol: RuntimeUniverse.TypeSymb, wireWith: DIKey) extends Association {
    override def format: String = s"""par ${symbol.name}: ${symbol.info.typeSymbol.name} = lookup($wireWith)"""
  }

  case class Method(context: DependencyContext.MethodContext, symbol: RuntimeUniverse.TypeSymb, wireWith: DIKey) extends Association {
