...
final class ConfigValueTest extends AnyWordSpec {
  ...
  "Config fields meta" should {
    ...
    "be unknown for config maps" in {
      ...
      assert(c.tpe == ConfigMetaType.TCaseClass(Seq("mymap" -> ConfigMetaType.TUnknown())))
    }

    "be unknown for config lists" in {
      val c = getConfTag(TestConfigReaders.listDefinition)
      assert(c.tpe == ConfigMetaType.TCaseClass(Seq("mylist" -> ConfigMetaType.TUnknown())))
    }
      ...
      assert(
        c.tpe == ConfigMetaType.TCaseClass(
          Seq(
            ...
            "optCustomObject" -> ConfigMetaType.TCaseClass(Seq("value" -> ConfigMetaType.TUnknown())),
          )
        )
      )
    }
    ...
    "be as expected for backticks" in {
      ...
      assert(c.tpe == ConfigMetaType.TCaseClass(Seq("boo-lean" -> ConfigMetaType.TUnknown())))
    }

    "be as expected for case classes with private fields" in {
      val c = getConfTag(TestConfigReaders.privateFieldsCodecDefinition)

      assert(c.tpe == ConfigMetaType.TCaseClass(Seq("private-custom-field-name" -> ConfigMetaType.TUnknown())))
    }
