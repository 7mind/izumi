package izumi.distage.config.interned.pureconfig.module.magnolia.semiauto

import izumi.distage.config.interned.pureconfig.module.magnolia.{EnumerationConfigWriterBuilder, MagnoliaConfigWriter}
import magnolia1.*
...
object writer {
  ...
  )(implicit builder: EnumerationConfigWriterBuilder[A]
  ): ConfigWriter[A] = builder.build(transformName)
}
