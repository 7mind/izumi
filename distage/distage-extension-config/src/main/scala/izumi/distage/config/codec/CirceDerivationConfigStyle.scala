...
object CirceDerivationConfigStyle {
  def deriveDecoder[A](implicit ev: CirceDerivationConfigStyle[A]): Decoder[A] = ev.value

  implicit def materialize[A]: CirceDerivationConfigStyle[A] = macro materializeImpl[A]

  final def materializeImpl[A: c.WeakTypeTag](c: blackbox.Context): c.Expr[CirceDerivationConfigStyle[A]] = {
    import c.universe._
