...
import com.github.pshirshov.izumi.idealingua.translator.togolang.types.GoLangImports

case class CSharpType (
                        id: TypeId,
  ...
  protected def renderPrimitiveType(primitive: Primitive, serialized: Boolean = false): String = primitive match {
    case Primitive.TBool => CSharpPrimitive("Boolean")
    case Primitive.TString => CSharpPrimitive("String")
    case Primitive.TInt8 => CSharpPrimitive("Byte")
    case Primitive.TInt16 => CSharpPrimitive("Int16")
    case Primitive.TInt32 => CSharpPrimitive("Int32")
    case Primitive.TInt64 => CSharpPrimitive("Int64")
    case Primitive.TFloat => CSharpPrimitive("Single")
    case Primitive.TDouble => CSharpPrimitive("Double")
    case Primitive.TUUID => CSharpPrimitive("Guid")
    case Primitive.TTime => if (serialized) "string" else "DateTime"
    case Primitive.TDate => if (serialized) "string" else "DateTime"
    case Primitive.TTs => if (serialized) "string" else "DateTime"
    case Primitive.TTsTz => if (serialized) "string" else "DateTime"
  }

  private val CSharpPrimitive : String => String = primitive => s"System.$primitive"
