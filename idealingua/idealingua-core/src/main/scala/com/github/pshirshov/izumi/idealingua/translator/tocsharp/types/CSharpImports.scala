...
case class CSharpImports(imports: List[CSharpImportRecord] = List.empty) {
  def renderImports(extra: String*): String = {
    //     Exclude extra ones which are already included into the import
    val combined = imports.map(i => i.renderImport()) ++ extra.toList.filterNot(e => imports.exists(p => p.pkg.mkString(".") == e)).map(e => "\"" + e + "\"")
    combined.map(lib => s"using $lib;").mkString("\n")
  }

  def findImport(id: TypeId): Option[CSharpImportRecord] = {
...
object CSharpImports {
  def apply(imports: List[CSharpImportRecord]): CSharpImports =
    new CSharpImports(imports)

  def apply(definition: TypeDef, fromPkg: Package, extra: List[CSharpImportRecord] = List.empty): CSharpImports =
    CSharpImports(fromDefinition(definition, fromPkg, extra))

  def apply(i: Service, fromPkg: Package, extra: List[CSharpImportRecord]): CSharpImports =
    CSharpImports(fromService(i, fromPkg, extra))
  ...
  protected def fromTypes(types: List[TypeId], fromPkg: Package, extra: List[CSharpImportRecord] = List.empty): List[CSharpImportRecord] = {
    ...
    packages.zipWithIndex.map { case (tp, index) =>
      if (packages.exists(p => p._3.last == tp._3.last && p._1 != tp._1))
        CSharpImportRecord(tp._1, s"imp_$index", tp._3)
      else
        CSharpImportRecord(tp._1, tp._3.last, tp._3)
    } ++ extra
  }

  protected def collectTypes(id: TypeId): List[TypeId] = {
    id match {
  ...
  protected def fromDefinition(definition: TypeDef, fromPkg: Package, extra: List[CSharpImportRecord] = List.empty): List[CSharpImportRecord] = {
    val types = collectTypes(definition)
    fromTypes(types, fromPkg, extra)
  }

  protected def fromService(svc: Service, fromPkg: Package, extra: List[CSharpImportRecord] = List.empty): List[CSharpImportRecord] = {
