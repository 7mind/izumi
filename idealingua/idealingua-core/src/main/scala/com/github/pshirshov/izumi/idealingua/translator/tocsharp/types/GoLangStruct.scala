...
case class GoLangStruct(
                          ...
                          fields: List[CSharpField] = List.empty,
                          imports: CSharpImports = CSharpImports(List.empty),
                          ts: Typespace = null,
  ...
  private def getInterfaceFields(iid: InterfaceId): List[CSharpField] = {
    val fields = ts.structure.structure(iid)
    fields.all.map(f => CSharpField(f.field.name, CSharpType(f.field.typeId, imports, ts), name, imports, ts))
  }
  ...
  private def isFieldPolymorph(f: CSharpField): Boolean = {
    f.tp.id.isInstanceOf[InterfaceId] || f.tp.id.isInstanceOf[AdtId] || f.tp.id.isInstanceOf[IdentifierId] || f.tp.id.isInstanceOf[DTOId]
  }

  private def getRefinedFields(fields: List[CSharpField], polymorph: Boolean): List[CSharpField] = {
    if (polymorph) {
  ...
  private def renderPolymorphSerializedVar(f: CSharpField): String = {
    f.tp.id match {
      case _: InterfaceId =>
  ...
  private def renderSerializedVar(f: CSharpField): String = {
    f.tp.id match {
  ...
  def renderSlice(method: String, slice: String, fields: List[CSharpField]): String = {
  ...
  def renderLoadSlice(method: String, slice: String, fields: List[CSharpField]): String = {
