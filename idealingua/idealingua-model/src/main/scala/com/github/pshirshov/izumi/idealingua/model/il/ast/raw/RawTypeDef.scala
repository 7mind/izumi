...
trait RawPositioned {
  def updatePosition(start: Int, stop: Int): RawPositioned
...
sealed trait RawTypeDef extends RawPositioned {
  ...
  override def updatePosition(start: Int, stop: Int): RawTypeDef = {
    updateMeta {
      meta =>
        meta.copy(position = InputPosition.Defined(start, stop))
    }
  }
}
