...
class TypespaceImpl(val domain: DomainDefinition) extends Typespace {
  ...
  protected def extractDependencies(definition: TypeDef): Seq[TypespaceImpl.Dependency] = {
    import TypespaceImpl._
    definition match {
      case _: Enumeration =>
        Seq.empty
