...
import com.github.pshirshov.izumi.idealingua.model.problems.{IDLDiagnostics, TyperError}
...
//          toIndefinite(d.id) -> transformSimpleId[RawDeclaredTypeName, TypeId](d.id)
//      }
//      .toMap ++ imported
//  }
//
...
//        typed.TypeDef.Alias(id = AliasId(TypePath(domainId, Seq.empty), id.name), target = fixId(src): TypeId, meta = fixMeta(c))
//
...
//  }
//
//  protected def toIndefinite(rawDeclaredTypeName: RawDeclaredTypeName): IndefiniteId = {
//    IndefiniteId(domainId.toPackage, rawDeclaredTypeName.name)
//  }
//
